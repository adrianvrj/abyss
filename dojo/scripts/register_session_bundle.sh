...
ARGUMENTS="${REFERRAL_PERCENTAGE}, ${REISSUABLE}, ${PRICE}, ${PAYMENT_TOKEN}, ${PAYMENT_RECEIVER}, \"${IMAGE_URI}\", ${ALLOWER}"
SERIALIZED="$(sncast utils serialize --abi-file "${ABI_FILE}" --function register_default_session_bundle --arguments "${ARGUMENTS}")"
CALLDATA="$(printf '%s\n' "${SERIALIZED}" | sed -n 's/^Calldata: \[//; s/\]$//; p' | tr ',' ' ')"

if [[ -z "${CALLDATA// }" ]]; then
  echo "Failed to serialize calldata for register_default_session_bundle." >&2
  exit 1
fi

echo "Invoking register_default_session_bundle on ${SETUP_ADDRESS}"
if [[ "${DRY_RUN:-0}" == "1" ]]; then
...
sncast --wait \
  ...
  --function register_default_session_bundle \
  --calldata ${CALLDATA}
