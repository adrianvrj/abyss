        ...
        <ModalWrapper onClose={onClose}>
            <div className="market-header">
                ...
                <button className="refresh-btn" onClick={handleRefresh} disabled={refreshing || currentScore < refreshCost}>
                    {refreshing ? "..." : <><RotateCw size={14} /> {refreshCost}</>}
                </button>
            </div>

            {loading ? (
                <div className="market-loading">Loading Market...</div>
            ) : (
                <div className="market-content">
                    <div className="market-carousel">
                        <button className="market-nav-arrow" onClick={handlePrev}><ChevronLeft /></button>

                        <div className="market-item-card">
                            {wasPurchased ? (
                                <div className="market-sold-out">
                                    <div className="market-sold-text">SOLD</div>
                                </div>
                            ) : (
                                <>
                                    ...
                                    <div className="market-item-info">
                                        ...
                                        <p className="market-desc">{currentItem?.description}</p>
                                        <div className="market-effect-badge">{getEffectDetails(currentItem!)}</div>
                                    </div>

                                    <button
                                        className={`market-buy-btn ${!canPurchase ? 'disabled' : ''}`}
                        ...
                        <button className="market-nav-arrow" onClick={handleNext}><ChevronRight /></button>
                    </div>
            ...
            <style dangerouslySetInnerHTML={{ __html: `
                .market-header { display: flex; justify-content: space-between; margin-bottom: 20px; }
                .balance-box { font-family: 'PressStart2P', monospace; color: #FF841C; display: flex; align-items: center; gap: 8px; font-size: 14px; }
                .refresh-btn { background: #333; border: 1px solid #FF841C; color: #FF841C; padding: 8px 12px; border-radius: 4px; cursor: pointer; display: flex; align-items: center; gap: 8px; font-family: 'PressStart2P', monospace; font-size: 10px; }
                .market-content { flex: 1; display: flex; flex-direction: column; align-items: center; }
                .market-carousel { display: flex; align-items: center; gap: 10px; width: 100%; justify-content: center; }
                .market-nav-arrow { background: none; border: none; color: #FF841C; font-size: 24px; cursor: pointer; }
                .market-item-card { background: rgba(0,0,0,0.6); border: 2px solid #FF841C; border-radius: 12px; padding: 20px; width: 280px; height: 400px; display: flex; flex-direction: column; align-items: center; justify-content: space-between; }
                .market-item-info { text-align: center; color: #fff; }
                .market-item-info h3 { font-family: 'PressStart2P', monospace; font-size: 14px; color: #FF841C; margin-bottom: 10px; }
                .market-desc { font-size: 12px; margin-bottom: 10px; color: #ccc; }
                .market-effect-badge { background: #FF841C; color: #000; padding: 4px 8px; border-radius: 4px; font-size: 10px; font-weight: bold; display: inline-block; }
                .market-buy-btn { width: 100%; padding: 12px; background: #FF841C; border: none; border-radius: 8px; font-family: 'PressStart2P', monospace; cursor: pointer; font-size: 12px; color: #000; }
                .market-buy-btn.disabled { background: #555; cursor: not-allowed; color: #888; }
                .market-pagination { margin-top: 10px; color: #666; font-family: 'PressStart2P', monospace; font-size: 10px; }
                .market-sold-out { flex: 1; display: flex; align-items: center; justify-content: center; color: #555; font-family: 'PressStart2P', monospace; font-size: 24px; }
                .market-loading { color: #fff; font-family: 'PressStart2P', monospace; text-align: center; margin-top: 40px; }
            ` }} />
        </ModalWrapper>
    );
}
