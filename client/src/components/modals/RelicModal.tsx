...
export default function RelicModal({
    ownedRelics, equippedRelic, relicIndex, isEquippingRelic,
    onClose, onSetRelicIndex, onEquipRelic,
}: RelicModalProps) {
        ...
        <ModalWrapper onClose={onClose} title="RELICS">
            {ownedRelics.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '40px 20px' }}>
                    ...
                    <p style={{ color: '#888', fontFamily: "'PressStart2P', monospace", fontSize: '10px', marginTop: '16px' }}>Mint relics in the shop!</p>
                </div>
            ) : (
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                        <button onClick={handlePrev} style={{ background: 'none', border: 'none', color: '#FF841C', fontSize: '24px', cursor: 'pointer', padding: '8px' }}>
                            <ChevronLeft />
                        </button>

                        <div style={{
                            background: 'rgba(0,0,0,0.6)', border: '2px solid #FF841C', borderRadius: '12px',
                            padding: '20px', width: '260px', minHeight: '340px',
                            ...
                            {currentRelic && (
                                <>
                                    ...
                                    <h3 style={{ fontFamily: "'PressStart2P', monospace", fontSize: '12px', color: '#FF841C', marginBottom: '10px', textAlign: 'center' }}>
                                        {currentRelic.name}
                                    </h3>
                                    <p style={{ fontSize: '11px', color: '#ccc', marginBottom: '8px', textAlign: 'center' }}>
                                        {RELIC_EFFECTS[currentRelic.relicId] || 'Special Effect'}
                                    </p>
                                    <p style={{ fontSize: '10px', color: '#888', marginBottom: '20px' }}>
                        ...
                        <button onClick={handleNext} style={{ background: 'none', border: 'none', color: '#FF841C', fontSize: '24px', cursor: 'pointer', padding: '8px' }}>
                            <ChevronRight />
                        </button>
                    </div>

                    <div style={{ marginTop: '15px', color: '#666', fontFamily: "'PressStart2P', monospace", fontSize: '10px' }}>
